...
- Use `@nestjs/mongoose` as the primary library for MongoDB operations
- Define schemas with `@Schema()` and properties with `@Prop()`
- Use `HydratedDocument` for document type safety
- Register models with `MongooseModule.forFeature()`
...
### **Dynamic Queries and Indices**
- Use Mongoose query methods (`find`, `findOne`, `updateMany`, etc.)
- Chain `.exec()` to return a proper promise and improve stack traces
- Define indices using `@Prop({ index: true })` or `@Schema({ indexes: [...] })`
- Use lean queries (`.lean()`) for read-only operations to improve performance
...
### **Unit Testing**
- Test services independently using mocks from `@nestjs/mongoose`
- Use `getModelToken(Schema.name)` to provide mock models
- Use `jest.fn()` to mock Mongoose query chains and document methods
...
describe('UsersService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        UsersService,
        {
          ...
          useValue: {
            new: jest.fn().mockReturnValue({ save: jest.fn() }),
            constructor: jest.fn(),
            find: jest.fn(),
            create: jest.fn(),
            exec: jest.fn(),
  ...
  it('should create a user', async () => {
    ...
    const savedUser = { ...createUserDto, _id: 'mock-id' };
